	...
	$.ajax({
		...
		success: function(datas){
			...
			if (result.ret.code==="200") {
					...
					if (result.data.awardlist[i].awardid == participation) {
						...
						if (result.data.awardlist[i].ptype == 1) {
							prizeHtml += '<input type="radio" class="prize-radio" name="prize_radio" checked onchange="changeRadioValue()" value="1"></div>'
							    + '<div class="thmeform-prize-p-5">大于</div>'
							    + '<div class="col-md-5 thmeform-prize-p-6">'
							    + '<input type="number" style="display:inline" class="form-control pinput" ptype="1" required="required" min="1" placeholder="数量" value="' + result.data.awardlist[i].awardnum + '"></div>'
							    + '<div class="thmeform-prize-p-5">票</div></div>'
							    + '<div class="cl"></div>'
							    + '<div>'
				...
				$("#adImgPath2").attr("src", result.data.adpicpath2);
				$("#adurl2").val(result.data.adurl2);
				$("#themeForm_aboutlink").val(result.data.abouturl);
				$("#themeForm_activitytype").val(result.data.activitytype);
				$("#themeForm_activitytype").trigger("chosen:updated");
				
				if (result.data.peoplerange == 1) {
					$("#join_teacher").prop("checked", true);
					$("#join_parent").prop("checked", false);
				} else if (result.data.peoplerange == 2) {
					$("#join_teacher").prop("checked", false);
					$("#join_parent").prop("checked", true);
				} else if (result.data.peoplerange == 3) {
					$("#join_teacher").prop("checked", true);
					$("#join_parent").prop("checked", true);
				}
				
				if (result.data.canguestvote == 1) {
					$("#canguestvote").prop("checked", true);
				}else{
					$("#canguestvote").prop("checked", false);
				}
				
				if (result.data.state == 10) {
					$("#themeForm_state").css("display", "none");
				} else {
					$('input[name="themeForm_state"][value="'+result.data.state+'"]').prop("checked", true);
				}
				
				$("#themeForm_businesslink").val(result.data.sponsorurl);
				$("#themeForm_buttonname").val(result.data.votebtnname);
				$("#themeForm_buttonurl").val(result.data.voteurl);
				$("#remark_content").val(result.data.remark);
...
function initCss() {
	var height = $(".img_upload_box").width()*5/9;
	$(".img_upload_box").css("height", height + "px");
	$(".img_upload_box .remind").css("height", height + "px");
	$(".img_upload_box .remind").css("line-height", height + "px");
}
 ...
 * @param preview 是否预览
 */
function saveAndPreview(preview) {
	var title = $("#title").val();
	if(title == null || title == ""){
	...
	var peoplerange = "";
	if ($('#join_teacher').is(":checked") && (!$('#join_parent').is(":checked"))) {
		peoplerange = 1;
	} else if ((!$('#join_teacher').is(":checked")) && $('#join_parent').is(":checked")) {
		peoplerange = 2;
	} else if ($('#join_teacher').is(":checked") && $('#join_parent').is(":checked")) {
		peoplerange = 3;
	}
	if(peoplerange == ""){
		PromptBox.alert("请勾选参与对象！");
		return false;
	}
	
	var canguestvote=1;
	if (!$('#canguestvote').is(":checked")){
		canguestvote=0;
	}
	
	var wxtAd="http://www.weixiaotong.com";
	var sponsorurl=$("#themeForm_businesslink").val();
	if( $.trim(sponsorurl)==''){
		sponsorurl=wxtAd;
	}
	var abouturl=$("#themeForm_aboutlink").val();
	if( $.trim(abouturl)==''){
		abouturl=wxtAd;
	}
	...
	var submitData = {
		...
		param:JSON.stringify({
			...
			voterule:voterule,
			abouturl:abouturl,
			activitytype:$("#themeForm_activitytype").val(),
			adpicpath1:$("#adImgPath1").attr("src"),
			adpicpath2:$("#adImgPath2").attr("src"),
			adurl1:$("#adurl1").val(),
			adurl2:$("#adurl2").val(),
			peoplerange:peoplerange,
			sponsorurl:sponsorurl,
			votebtnname:$("#themeForm_buttonname").val(),
			voteurl:$("#themeForm_buttonurl").val(),
			state:$('input[name="themeForm_state"]:checked').val(),
...
function getActivityTypeList(){
	var submitData = {
		...
		param: JSON.stringify({
			type:DICTTYPE.DICT_TYPE_GHHD_ACTIVITY_TYPE
		})
	};
	$.ajax({
		cache:false,
		...
		success: function(datas){
			...
			if (result.ret.code==="200") {
				...
				$("#themeForm_activitytype").append(dataList.join(''));
				$("#themeForm_activitytype").trigger("chosen:updated");
			} else {
				console.log(result.ret.code+":"+result.ret.msg);
			}
...
function loadCityList(){
	...
	$("#dls_list").find("option:selected").each(function(){
		if($(this).val()==7){
			citys=citys+"340000340100,340000340200,340000340300,340000340400,340000340500,340000340600,340000340700,340000340800,340000341000,340000341100,340000341200,340000341300,340000341400,340000341500,340000341600,340000341700,340000341800,";
		}else{
...
function goBack() {
	window.location.href = ctx + "/base/func/" + ApiParamUtil.APPID_GHHDZB_LIST_JUMP + "?appid=" + $("#appid").val();
}
