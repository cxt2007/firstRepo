;(function ($) {
	...
	$.confirm=function(o){
		...
		$.extend(dfop, o);
		var div=
			'<div class="loginOutTxt" title="'+dfop.title+'" >'
			+'<p><span class="'+dfop.level+'"></span>'
			+dfop.message
			+'</p>'
			+'</div>';
		...
		$('.loginOutTxt').siblings(".ui-dialog-buttonpane").children(".ui-dialog-buttonset").children().eq(0).addClass("butFocus");
	...
	$.fn.createDialog = function(o) {
		...
		function buttonStyle(docObj){
			var $nextBtn=docObj.closest(".ui-dialog").find(".ui-dialog-buttonpane").find("button:contains('确定')");
			var $saveBtn=docObj.closest(".ui-dialog").find(".ui-dialog-buttonpane").find("button:contains('保存')").first();
			var $clearBtn=docObj.closest(".ui-dialog").find(".ui-dialog-buttonpane").find("button:contains('确认')").first();
			var $searchBtn=docObj.closest(".ui-dialog").find(".ui-dialog-buttonpane").find("button:contains('查询')").first();
			if($nextBtn[0]){
				$nextBtn.addClass("butFocus");
			}else if($searchBtn[0]){
				$searchBtn.addClass("butFocus");
			}else if($saveBtn[0]){
				$saveBtn.addClass("butFocus");
			}else{
				$clearBtn.addClass("butFocus");
			}
		}
		function getAjax(url,docObj) {
			docObj.empty();
